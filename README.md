...
Mycelial does not poll or watch inboxes. Its generated, user-invoked launcher composes Herdr and Pi without moving process supervision into mailbox runtime code. After durable send or reply succeeds, the outer tool workflow automatically attempts a fixed, content-free Herdr wake-up for each delivered recipient. Notification results are reported separately and failure never invalidates durable mail. `agent_wake` remains available only for retrying a reported notification failure. Persona policy remains outside the mailbox layer. Immutable `claim-events/` history is explicitly deferred in v1; claim files retain current state and monotonic transition versions.
